...
import { replyComment, getsinglePost, likeComment, unlikeComment, likeReply, unlikeReply } from '../api/post';
...
export default class commentreply extends React.Component {
    ...
    _replyComment = async () => {
        ...
        if (content) {
            try {
                ...
                const res = await replyComment(body, token)
    ...
    _getSinglePost = async () => {
        try {
            ...
            const comment = this.props.navigation.state.params?.comment
            const res = await getsinglePost(comment?.postId, token)
            const filterComment = res.data.post.comments?.filter(e => e._id === comment?._id)
            this.setState({ comment: filterComment[0], replies: filterComment[0]?.reply })
        } catch (error) {
            console.log(error)
            Toast.show("Something went wrong...", {
                duration: Toast.durations.LONG,
    ...
    _likeReply = async (id) => {
        ...
        try {
            const res = await likeReply(id, token)
            this._getSinglePost()
    ...
    _unlikeReply = async (id) => {
        ...
        try {
            const res = await unlikeReply(id, token)
            this._getSinglePost()
                    ...
                    <View style={{ marginTop: wp('5%'), padding: 30 }}>
                        ...
                        <View style={{ flexDirection: 'row' }}>
                            ...
                            <View style={{ width: '45%', marginLeft: 10, marginRight: 10 }}>
                                <View>
                                    <Text style={{ fontSize: 20, fontWeight: 'bold', marginTop: 0 }}>Replies</Text>
                                </View>
                            </View>
                        </View>
...
{/* 
                        ...
                        </View>
                        {
                            replies?.map((comment, index) => (
                                <View key={index} style={{ marginTop: 40, width: '90%', alignItems: 'flex-end' }}>
                                    <View style={{ width: '90%', backgroundColor: '#EAEAEA', padding: 10, borderRadius: 10 }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <View style={{ width: '80%' }}>
                                                <Image style={{ width: 50, height: 50, marginTop: -40, borderWidth: 1, borderColor: "#DDB937", borderRadius: 500 }} source={{ uri: comment?.user?.avatar }} />
                                    ...
                                    <View style={{ flexDirection: 'row', padding: 10 }}>
                                        ...
                                        <TouchableOpacity onPress={() => this.checkMyLike(comment?.likes) ? this._unlikeReply(comment?._id) : this._likeReply(comment?._id)} style={{ width: '20%' }}>
                                            <Text style={{ fontSize: 13, color: '#9B9B9B', paddingLeft: 10, paddingTop: 4, fontWeight: 'bold' }}>{this.checkMyLike(comment?.likes) ? "Unlike" : "Like"}</Text>
                                        </TouchableOpacity>
                            ...
                            </View>
                            {
                                this.state.recording ?
                                    ...
                                    <>
                                        ...
                                        <TouchableOpacity onPress={() => this._replyComment()} style={{ width: '10%' }}>
